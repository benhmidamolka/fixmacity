        ...
        return (
          <div key={step} className="flex items-start gap-2.5">
            <div className="flex flex-col items-center">
              <div className={`w-3.5 h-3.5 rounded-full border-2 flex-shrink-0 mt-0.5 ${done ? 'bg-[#1557FF] border-[#1557FF]' : 'bg-white border-slate-300'}`} />
              {i < TIMELINE_STEPS.length-1 && <div className={`w-0.5 h-5 ${i < active-1 ? 'bg-[#1557FF]' : 'bg-slate-200'}`} />}
            </div>
            <div>
              <p className={`text-xs font-semibold leading-tight ${done ? 'text-[#0A1628]' : 'text-slate-400'}`}>{step}</p>
              {h?.changed_at && (
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(!open)}
        className="flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-700 hover:border-[#1557FF] transition-all min-w-32">
        {value === options[0] ? label : value}
        <ChevronDown className="w-4 h-4 ml-auto" />
      </button>
      {open && (
        <div className="absolute top-full mt-1 left-0 bg-white border border-slate-200 rounded-xl shadow-lg z-[1100] min-w-44 py-1">
          {options.map(o => (
            <button key={o} onClick={() => { onChange(o); setOpen(false) }}
              className={`w-full text-left px-4 py-2 text-sm transition-colors ${
                value===o ? 'text-[#1557FF] font-bold bg-blue-50' : 'text-slate-700 hover:bg-slate-50'
              }`}>
              {o}
            </button>
          ))}
  ...
  return (
    <CitizenLayout>
      <div className="h-[calc(100vh-64px)] flex flex-col">
        ...
        {/* ── Filter bar ── */}
        <div className="bg-white border-b border-slate-100 px-4 py-3 flex items-center gap-3 z-10 shadow-sm flex-wrap">
          ...
          <div className="relative flex-1 min-w-52">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 placeholder-slate-400 outline-none focus:border-[#1557FF] focus:bg-white transition-all"
            />
            {/* Autocomplete dropdown */}
            {showSugg && suggestions.length > 0 && (
              <div className="absolute top-full mt-1 left-0 right-0 bg-white border border-slate-200 rounded-xl shadow-xl z-[1200] overflow-hidden">
                {suggestions.map((s, i) => (
                  <button key={i}
                    onMouseDown={() => handleSelectSuggestion(s)}
                    className="w-full text-left px-4 py-2.5 text-sm hover:bg-blue-50 hover:text-[#1557FF] transition-colors flex items-center gap-2 border-b border-slate-50 last:border-0">
        ...
        <div className="flex-1 relative overflow-hidden">
          ...
          {/* Pins / Heatmap toggle */}
          <div className="absolute top-3 z-[999] flex items-center gap-2 bg-white/95 backdrop-blur-sm border border-slate-200 rounded-xl px-3 py-2 shadow-md"
            style={{ right: selected ? '336px' : '16px' }}>
            <span className={`text-xs font-semibold ${!heatmap ? 'text-[#1557FF]' : 'text-slate-400'}`}>Pins</span>
            <button onClick={() => setHeatmap(!heatmap)}
              className={`relative w-10 h-5 rounded-full transition-all ${heatmap ? 'bg-[#1557FF]' : 'bg-slate-200'}`}>
              <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-all ${heatmap ? 'left-5' : 'left-0.5'}`} />
            </button>
            <span className={`text-xs font-semibold ${heatmap ? 'text-[#1557FF]' : 'text-slate-400'}`}>Heatmap</span>
          </div>
          ...
          <div className="absolute top-3 z-[999] left-1/2 -translate-x-1/2">
            ...
            <button
              ...
              className="flex items-center gap-2 bg-white/95 backdrop-blur-sm border border-slate-200 rounded-full px-4 py-2 shadow-md text-sm font-bold text-slate-700 hover:bg-blue-50 hover:border-[#1557FF] hover:text-[#1557FF] transition-all">
          ...
          {/* Legend */}
          <div className="absolute bottom-6 left-4 bg-white/95 backdrop-blur-sm rounded-2xl px-4 py-3 shadow-lg border border-slate-100 z-[999]">
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Statut des signalements</p>
            {LEGEND.map(item => (
          ...
          <div className="absolute bottom-6 flex flex-col gap-1 z-[999]"
            style={{ right: selected ? '336px' : '16px' }}>
            <button className="w-9 h-9 bg-white rounded-xl shadow-md border border-slate-200 flex items-center justify-center text-slate-700 font-bold hover:bg-slate-50 text-lg">+</button>
            <button className="w-9 h-9 bg-white rounded-xl shadow-md border border-slate-200 flex items-center justify-center text-slate-700 font-bold hover:bg-slate-50 text-lg">−</button>
            <button
              onClick={() => {
                navigator.geolocation?.getCurrentPosition(p => setFlyTo([p.coords.latitude, p.coords.longitude]))
              }}
              className="w-9 h-9 bg-white rounded-xl shadow-md border border-slate-200 flex items-center justify-center text-[#1557FF] hover:bg-slate-50 mt-1">
