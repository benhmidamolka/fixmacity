...
import {
  ...
  Calendar, Check, AlertCircle, History
} from 'lucide-react'
...
const AgentDashboard: React.FC = () => {
  ...
  // Auto-open if redirected from mission URL
  useEffect(() => {
  ...
  const fetchTasks = async () => {
    try {
      const res = await fetch('http://localhost:5000/api/agent/declarations', {
  ...
  const handleAccept = async (id: string, date_debut?: string) => {
    try {
      const res = await fetch(`http://localhost:5000/api/agent/declarations/${id}/accept`, {
        method: 'POST',
        headers: { 
  ...
  const handleRefuse = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/agent/declarations/${refuseModal}/refuse`, {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          Authorization: `Bearer ${localStorage.getItem('fmc_token')}` 
...
const TaskDetail: React.FC<{ 
  ...
  const [activeStep, setActiveStep] = useState(task.status === 'assignee_agent' ? 'decision' : 'intervene')
  ...
  const handleResolve = async () => {
    // Validation: date_fin must be >= date_debut
    ...
    try {
      ...
      if (photo) {
        ...
        await fetch(`http://localhost:5000/api/agent/declarations/${task.id}/photo`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${localStorage.getItem('fmc_token')}` },
          body: formData
        })
      ...
      // 2. Mark as resolved
      const res = await fetch(`http://localhost:5000/api/agent/declarations/${task.id}/resolve`, {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          Authorization: `Bearer ${localStorage.getItem('fmc_token')}` 
