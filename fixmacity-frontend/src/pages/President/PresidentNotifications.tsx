...
const NOTIF_TYPES: Record<string, { icon: React.ReactNode; color: string; bg: string; label: string }> = {
  URGENT_DECLARATION: { icon: <AlertTriangle className="w-5 h-5"/>, color: '#EF4444', bg: 'bg-rose-50 dark:bg-rose-500/10', label: 'CRITIQUE' },
  NEW_DECLARATION:    { icon: <FileText className="w-5 h-5"/>,     color: '#1557FF', bg: 'bg-blue-50 dark:bg-blue-500/10', label: 'SIGNALEMENT' },
  STATUS_CHANGE:      { icon: <Zap className="w-5 h-5"/>,          color: '#F59E0B', bg: 'bg-amber-50 dark:bg-amber-500/10', label: 'WORKFLOW' },
  ASSIGNED_CHEF:      { icon: <UserPlus className="w-5 h-5"/>,     color: '#10B981', bg: 'bg-emerald-50 dark:bg-emerald-500/10', label: 'MISSION' },
  DECLARATION_REJECTED: { icon: <AlertTriangle className="w-5 h-5"/>, color: '#EF4444', bg: 'bg-rose-50 dark:bg-rose-500/10', label: 'ALERTE' },
  PROPOSITION_VOTE:   { icon: <ThumbsUp className="w-5 h-5"/>,    color: '#8B5CF6', bg: 'bg-violet-50 dark:bg-violet-500/10', label: 'CONSULTATION' },
  SYSTEM:             { icon: <Settings className="w-5 h-5"/>,     color: '#64748B', bg: 'bg-slate-50 dark:bg-slate-800', label: 'SYSTÈME' },
}
...
const KpiCard: React.FC<{ label: string; value: number; color: string; icon: React.ReactNode; sub: string }> = ({ label, value, color, icon, sub }) => (
  <div className="group bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 border border-slate-200/60 dark:border-slate-800 hover:border-blue-400/50 dark:hover:border-blue-500/50 hover:shadow-[0_20px_50px_rgba(0,0,0,0.04)] transition-all duration-500 relative overflow-hidden">
    <div className="absolute top-0 right-0 w-32 h-32 bg-slate-50/50 dark:bg-slate-800/20 rounded-bl-[5rem] -mr-10 -mt-10 group-hover:bg-blue-50/50 dark:group-hover:bg-blue-500/10 transition-colors duration-500" />
  ...
  if (loading) return (
    <PresidentLayout title="Centre de Notifications">
      <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">Synchronisation des flux...</p>
      </div>
    </PresidentLayout>
  )
  ...
  return (
    <PresidentLayout title="Centre de Notifications">
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        {/* Main Content Area */}
        <div className="bg-white dark:bg-slate-900 rounded-[3.5rem] border border-slate-200/60 dark:border-slate-800 shadow-[0_40px_80px_-20px_rgba(0,0,0,0.04)] overflow-hidden">
          ...
          <div className="px-10 py-8 border-b border-slate-100 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-800/20 flex flex-col xl:flex-row xl:items-center justify-between gap-6">
            <div className="flex items-center gap-3 bg-white dark:bg-slate-900 p-1.5 rounded-[1.5rem] border border-slate-200 dark:border-slate-800 shadow-sm self-start">
              {[
                ...
                <button key={f.key} onClick={() => setFilter(f.key)}
                  className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                    filter === f.key
                      ? 'bg-slate-900 dark:bg-[#1557FF] text-white shadow-xl'
            ...
            <div className="flex items-center gap-4 flex-wrap">
              <div className="relative group">
                <Search className="absolute left-6 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300 dark:text-slate-600 group-focus-within:text-[#1557FF] transition-colors" />
                <input 
                  type="text"
                  placeholder="Analyser les alertes..."
                  value={searchQuery}
                  onChange={e => setSearchQuery(e.target.value)}
                  className="pl-14 pr-6 h-14 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl text-sm font-bold text-[#0A1628] dark:text-white placeholder-slate-300 dark:placeholder-slate-600 focus:outline-none focus:ring-4 focus:ring-blue-50 dark:focus:ring-blue-500/10 focus:border-[#1557FF]/30 transition-all w-72"
                />
              </div>

              <select value={typeF} onChange={e => setTypeF(e.target.value)}
                className="h-14 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl px-6 py-2.5 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 outline-none cursor-pointer hover:border-[#1557FF]/30 transition-all appearance-none pr-12 min-w-[200px]"
          ...
          <div className="divide-y divide-slate-50 dark:divide-slate-800">
            {filtered.length === 0 ? (
              <div className="py-32 text-center">
                <div className="w-24 h-24 rounded-[2.5rem] bg-slate-50 dark:bg-slate-800/50 flex items-center justify-center mx-auto mb-8 text-slate-200 dark:text-slate-700 shadow-inner">
                  <Bell className="w-10 h-10" />
                </div>
                <h3 className="text-2xl font-black text-[#0A1628] dark:text-white tracking-tight">Signal Néant</h3>
                <p className="text-slate-400 dark:text-slate-500 font-medium mt-2 italic">Aucune alerte correspondante dans ce segment.</p>
              </div>
            ) : (
              filtered.map(n => {
                ...
                return (
                  <div key={n.id}
                    className={`flex items-start gap-8 px-10 py-8 transition-all cursor-pointer group relative ${
                      !n.is_read ? 'bg-blue-50/30 dark:bg-blue-500/5' : 'hover:bg-slate-50 dark:hover:bg-slate-800/50'
                    ...
                    <div
                      ...
                      className={`w-7 h-7 rounded-xl border-2 mt-1.5 flex-shrink-0 flex items-center justify-center cursor-pointer transition-all ${
                        isSelected ? 'border-[#1557FF] bg-[#1557FF] shadow-lg shadow-blue-500/20' : 'border-slate-200 dark:border-slate-700 group-hover:border-[#1557FF]/30 bg-white dark:bg-slate-800'
                      }`}>
                      {isSelected && <Check className="w-4 h-4 text-white font-black"/>}
                    </div>

                    {/* Category Icon */}
                    <div className="w-16 h-16 rounded-[1.5rem] flex items-center justify-center flex-shrink-0 shadow-sm border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500"
                    ...
                    <div className="flex-1 min-w-0 pt-1">
                      <div className="flex items-center justify-between gap-6 mb-2">
                        <div className="flex items-center flex-wrap gap-4">
                          <h3 className={`text-lg tracking-tight leading-none ${!n.is_read ? 'font-black text-[#0A1628] dark:text-white' : 'font-bold text-slate-500 dark:text-slate-400'}`}>
                            {n.title}
                          </h3>
                          ...
                          {!n.is_read && (
                            <div className="flex items-center gap-1.5 bg-blue-50 dark:bg-blue-500/10 px-2.5 py-1 rounded-lg">
                      ...
                      <p className={`text-base leading-relaxed max-w-4xl ${!n.is_read ? 'text-slate-600 dark:text-slate-300 font-medium' : 'text-slate-400 dark:text-slate-500 italic'}`}>
                        {n.body}
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-all flex-shrink-0 translate-x-4 group-hover:translate-x-0 pt-2">
                      {!n.is_read && (
                        <button
                          ...
                          className="w-12 h-12 rounded-2xl flex items-center justify-center bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-[#1557FF] hover:bg-blue-50 dark:hover:bg-blue-500/10 hover:border-blue-200 dark:hover:border-blue-500/30 transition-all shadow-sm"
                      ...
                      <button
                        ...
                        className="w-12 h-12 rounded-2xl flex items-center justify-center bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-300 dark:text-slate-600 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 hover:border-rose-100 dark:hover:border-rose-500/30 transition-all shadow-sm"
                        title="Supprimer">
                        <Trash2 className="w-5 h-5"/>
                      </button>
                      <div className="w-12 h-12 rounded-2xl flex items-center justify-center bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-300 dark:text-slate-600 hover:text-slate-600 dark:hover:text-slate-300 transition-all shadow-sm">
          ...
          {/* Footer Info */}
          <div className="px-10 py-10 bg-slate-50/50 dark:bg-slate-800/30 border-t border-slate-100 dark:border-slate-800 flex flex-col items-center justify-center">
