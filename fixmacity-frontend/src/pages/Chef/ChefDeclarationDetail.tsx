...
function getPriorityIcon(p: string) {
  ...
  if (['haute','high','urgent','urgente'].includes(lo))
    return <ArrowUp size={12} className="text-red-500" />
  if (['moyenne','medium'].includes(lo))
...
function getStatusCfg(s: string) {
  return STATUS_CONFIG[s] || { label: s, color: '#64748b', bg: '#f1f5f9', dot: '#94a3b8' }
}
...
// ── Accept modal (inline, no external component) ──────────────────────────────
function AcceptModal({ decl, agents, onClose, onDone }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      style={{ background: 'rgba(10,22,40,.55)', backdropFilter: 'blur(6px)' }}
      onClick={onClose}>
      <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden"
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-slate-900 dark:text-slate-100">Accepter et assigner un agent</p>
              <p className="text-[10px] text-slate-400 truncate max-w-52">{decl.title}</p>
            </div>
          </div>
          <button onClick={onClose} className="w-7 h-7 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-400 hover:text-slate-600 dark:hover:text-slate-300">
            <X size={13} />
          </button>
        </div>
        <div className="p-5 space-y-3">
          {active.length === 0 ? (
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {active.map(a => {
                const isOverloaded = a.workload >= maxTasks;
                return (
                  <label key={a.id}
                    ...
                    <input type="radio" name="agent" value={a.id} checked={agentId === a.id}
                      onChange={() => setAgentId(a.id)} className="sr-only" />
                    <div className="w-9 h-9 rounded-xl flex items-center justify-center font-black text-xs text-white flex-shrink-0"
                      style={{ background: isOverloaded ? '#ef4444' : '#3b82f6' }}>
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <div className="flex-1 h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                          <div className="h-full rounded-full" style={{
                            ...
                            background: isOverloaded ? '#ef4444' : a.workload >= Math.ceil(maxTasks / 2) ? '#f59e0b' : '#22c55e'
                          }} />
                        </div>
                        <span className="text-[10px] text-slate-400">{a.workload}/{maxTasks}</span>
                      </div>
                    </div>
                    {isOverloaded && <span className="text-[9px] font-black text-red-500 border border-red-200 dark:border-red-500/30 rounded-full px-1.5 py-0.5">Chargé</span>}
          ...
          {warning && <div className="flex gap-2 px-3 py-2 bg-amber-50 dark:bg-amber-500/10 border border-amber-100 dark:border-amber-500/20 rounded-xl text-xs text-amber-700 dark:text-amber-400"><AlertTriangle size={13} className="flex-shrink-0 mt-0.5" />{warning}</div>}
          {error   && <div className="flex gap-2 px-3 py-2 bg-red-50 dark:bg-red-500/10 border border-red-100 dark:border-red-500/20 rounded-xl text-xs text-red-700 dark:text-red-400"><AlertCircle size={13} className="flex-shrink-0 mt-0.5" />{error}</div>}
        </div>
        <div className="px-5 pb-5 flex gap-3">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-sm font-semibold text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800">Annuler</button>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden"
        ...
        <div className="p-5 space-y-3">
          <div className="bg-red-50 dark:bg-red-500/10 border border-red-100 dark:border-red-500/20 rounded-xl p-3 text-xs text-red-700 dark:text-red-400">Le Président sera notifié et pourra réassigner ce dossier.</div>
          <textarea value={reason} onChange={e => setReason(e.target.value)} rows={4}
            placeholder="Motif du refus (obligatoire)…"
            className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-sm outline-none focus:border-red-400 dark:focus:border-red-500 resize-none text-slate-700 dark:text-slate-300 placeholder-slate-400 dark:placeholder-slate-600" />
            ...
            <div className="px-3 pb-2 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-2 px-2 py-1.5 bg-slate-50 dark:bg-slate-800 rounded-lg">
                ...
                <input value={q} onChange={e => setQ(e.target.value)} autoFocus
                  ...
                  className="flex-1 text-xs bg-transparent outline-none text-slate-700 dark:text-slate-300 placeholder-slate-400 dark:placeholder-slate-600" />
              </div>
            </div>
          )}
          <div className="max-h-52 overflow-y-auto py-1">
            {filtered.map(opt => (
              <button key={opt.value}
                ...
                {opt.label}
                {value === opt.value && <Check size={12} className="ml-auto text-slate-900 dark:text-slate-100" />}
              </button>
            ))}
          </div>
        </div>
...
function SortIcon({ col, sort }: { col: string; sort: { col: string; dir: 'asc' | 'desc' } }) {
  ...
  return sort.dir === 'asc'
    ? <ChevronUp size={12} className="text-slate-600 dark:text-slate-300" />
    : <ChevronDown size={12} className="text-slate-600 dark:text-slate-300" />
}
...
const ChefDeclarations: React.FC = () => {
  ...
  const [visibleCols,  setVisibleCols]  = useState({
    ref: true, project: false, assignee: true, dueDate: true, status: true, priority: true
  })
  ...
  const load = useCallback(async () => {
    ...
    if (dRes.ok) {
      ...
      setDeclarations(Array.isArray(d) ? d : d.declarations || [])
    }
    if (aRes.ok) {
  ...
  const handleAddComment = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API}/chef/declarations/${routeId}/comments`, {
        method: 'POST',
        headers: hjson(),
        body: JSON.stringify({ content: commentText.trim(), channel: 'interne' })
      })
      if (res.ok) {
        setCommentText('')
  ...
  const handleSaveAssignment = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const d = await res.json()
        setDetailDecl((prev: any) => prev ? { ...prev, ...d.declaration, assigned_agents: d.assigned_agents } : null)
        await fetchDetail(routeId)
        load()
      }
    } catch (err) {
      console.error(err)
  ...
  const handleExport = async () => {
    ...
    a.href = url; a.download = `declarations-chef-${new Date().toISOString().slice(0, 10)}.csv`
  ...
  const statusOpts = [
    ...
    { value: 'assignee_chef',  label: 'À traiter',        color: '#d97706' },
    { value: 'assignee_agent', label: 'Chez l\'agent',    color: '#7c3aed' },
    { value: 'en_cours',       label: 'En cours',         color: '#1d4ed8' },
    { value: 'resolue',        label: 'Résolue',          color: '#15803d' },
    { value: 'cloturee',       label: 'Clôturée',         color: '#475569' },
    { value: 'refusee_chef',   label: 'Refusée',          color: '#dc2626' },
    { value: 'refusee_agent',  label: 'Refus agent',      color: '#b91c1c' },
  ]
  ...
  const filtered = declarations.filter(d => {
    ...
    let av: any = a[sort.col as keyof Decl] ?? ''
    let bv: any = b[sort.col as keyof Decl] ?? ''
  ...
  const anyActive = statusFilter !== 'all' || prioFilter !== 'all' || agentFilter !== 'all'
  ...
  const agentName = (d: Decl) => {
    if (d.users) return `${d.users.first_name} ${d.users.last_name}`
      ...
      <div className="flex flex-col h-full min-h-0 bg-white dark:bg-slate-950">
          ...
          <div className="flex items-center justify-between mb-1">
            <div>
              ...
              <p className="text-sm text-slate-400 dark:text-slate-500 mt-0.5">
                Liste de toutes les déclarations de votre département.
              </p>
            </div>
            <button
              onClick={() => navigate('/chef/tasks')}
              className="flex items-center gap-2 px-4 py-2.5 bg-slate-900 dark:bg-white hover:bg-slate-800 dark:hover:bg-slate-100 text-white dark:text-slate-900 rounded-xl text-sm font-bold transition-all shadow-sm">
        ...
        <div className="flex-shrink-0 px-6 py-3 flex items-center gap-2 flex-wrap border-b border-slate-100 dark:border-slate-800">
          ...
          <div className="relative">
            ...
            <input value={search} onChange={e => { setSearch(e.target.value); setPage(1) }}
              ...
              className="pl-8 pr-4 py-1.5 text-xs bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-full outline-none focus:border-slate-400 dark:focus:border-slate-500 w-48 text-slate-700 dark:text-slate-300 placeholder-slate-400 dark:placeholder-slate-600 transition-all" />
          </div>

          {/* Filter pills */}
          <FilterDropdown
            label="Statut"
            options={statusOpts.filter(o => o.value !== 'all')}
            value={statusFilter}
            onChange={v => { setStatusFilter(v); setPage(1) }} />

          <FilterDropdown
            label="Priorité"
            options={prioOpts.filter(o => o.value !== 'all')}
            value={prioFilter}
            onChange={v => { setPrioFilter(v); setPage(1) }}
            searchable />

          <FilterDropdown
            label="Agent"
            options={agentOpts.filter(o => o.value !== 'all')}
            value={agentFilter}
          ...
          {anyActive && (
            <button
              onClick={() => { setStatusFilter('all'); setPrioFilter('all'); setAgentFilter('all'); setSearch(''); setPage(1) }}
              className="flex items-center gap-1.5 pl-3 pr-3 py-1.5 rounded-full border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-xs font-bold text-slate-600 dark:text-slate-400 hover:border-slate-300">
              Reset <X size={11} />
            </button>
          )}

          <div className="ml-auto flex items-center gap-2">
            <button onClick={load} className="p-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-all">
              <RefreshCw size={13} className={loading ? 'animate-spin' : ''} />
            </button>
            <button onClick={handleExport} className="p-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-all">
              <Download size={13} />
            </button>
            ...
            <div className="relative">
              <button onClick={() => setColsOpen(!colsOpen)}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-xs font-bold text-slate-600 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-600">
                ...
                <div className="absolute top-full mt-1.5 right-0 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-xl z-40 w-44 py-2">
                  {Object.entries({ ref:'Référence', project:'Projet', assignee:'Agent assigné', dueDate:'Date soumission', status:'Statut', priority:'Priorité' }).map(([k, v]) => (
                    <label key={k} className="flex items-center gap-2.5 px-4 py-2 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                      <input type="checkbox" checked={visibleCols[k as keyof typeof visibleCols]}
        ...
        <div className="flex-1 min-h-0 overflow-auto">
          <table className="w-full min-w-[800px]">
            <thead className="bg-slate-50 dark:bg-slate-900 sticky top-0 z-10">
              <tr className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest border-b border-slate-100 dark:border-slate-800">
                ...
                {visibleCols.ref && <th className="px-4 py-3 text-left cursor-pointer" onClick={() => toggleSort('ref_citoyen')}>
                  <div className="flex items-center gap-1">Réf. <SortIcon col="ref_citoyen" sort={sort} /></div>
                </th>}
                {visibleCols.assignee && <th className="px-4 py-3 text-left">Agent assigné</th>}
                {visibleCols.dueDate && <th className="px-4 py-3 text-left cursor-pointer" onClick={() => toggleSort('created_at')}>
                  <div className="flex items-center gap-1">Date soumission <SortIcon col="created_at" sort={sort} /></div>
                </th>}
                {visibleCols.status && <th className="px-4 py-3 text-left cursor-pointer" onClick={() => toggleSort('status')}>
                  <div className="flex items-center gap-1">Statut <SortIcon col="status" sort={sort} /></div>
                </th>}
                {visibleCols.priority && <th className="px-4 py-3 text-left cursor-pointer" onClick={() => toggleSort('priority')}>
                  <div className="flex items-center gap-1">Priorité <SortIcon col="priority" sort={sort} /></div>
                </th>}
                  ...
                  <td colSpan={8} className="px-6 py-16 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-sm font-bold text-slate-400 dark:text-slate-500">
                        {search || anyActive ? 'Aucun résultat pour ces filtres.' : 'Aucune déclaration dans votre département.'}
                      </p>
                    </div>
                  </td>
                </tr>
                  ...
                  <tr key={d.id}
                    ...
                    <td className="px-4 py-3.5 max-w-xs">
                      <div className="flex items-center gap-2.5">
                        <div className="w-7 h-7 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-sm flex-shrink-0">
                        ...
                        <div className="min-w-0">
                          ...
                          {d.category && <p className="text-[10px] text-slate-400 dark:text-slate-500 truncate">{d.category}</p>}
                        </div>
                      </div>
                    </td>
                      ...
                      <td className="px-4 py-3.5" onClick={e => e.stopPropagation()}>
                        {an ? (
                          ...
                          d.status === 'assignee_chef' ? (
                            <div className="flex items-center gap-2">
                              <button
                                onClick={e => { e.stopPropagation(); setAcceptTarget(d) }}
                                className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[10px] font-black border border-emerald-100 dark:border-emerald-500/20 hover:bg-emerald-100 dark:hover:bg-emerald-500/20 transition-all">
                                <UserCheck size={11} /> Accepter
                              </button>
                              <button
                                onClick={e => { e.stopPropagation(); setRefuseTarget(d) }}
                                className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 text-[10px] font-black border border-red-100 dark:border-red-500/20 hover:bg-red-100 dark:hover:bg-red-500/20 transition-all">
                                <XCircle size={11} /> Refuser
                              </button>
                            </div>
                          ) : (
                            <span className="text-[10px] text-slate-400 dark:text-slate-500 italic">Non assigné</span>
                          )
                        )}
                      </td>
                    )}
                    ...
                    {visibleCols.dueDate && (
                      <td className="px-4 py-3.5">
                        <span className="text-xs text-slate-600 dark:text-slate-400">{fmtDate(d.created_at)}</span>
                      </td>
                    )}
                    ...
                    {visibleCols.priority && (
                      <td className="px-4 py-3.5">
                        <span className="inline-flex items-center gap-1 text-[10px] font-black px-2.5 py-1 rounded-full"
                          ...
                          {getPriorityIcon(d.priority)} {pri.label}
                        </span>
                      </td>
                    )}
                    ...
                    <td className="px-4 py-3.5" onClick={e => e.stopPropagation()}>
                      <div className="relative">
                        <button
                          ...
                          className="w-7 h-7 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center justify-center text-slate-400 dark:text-slate-500 opacity-0 group-hover:opacity-100 transition-all">
                          <MoreHorizontal size={14} />
                        </button>
                        {actionMenu === d.id && (
                          <div className="absolute right-0 top-full mt-1 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl shadow-xl z-50 w-44 py-1 overflow-hidden"
                            ...
                            {d.status === 'assignee_chef' && (
                              <>
                                <button onClick={() => { setAcceptTarget(d); setActionMenu(null) }}
                                  className="w-full flex items-center gap-2.5 px-4 py-2 text-xs font-bold text-emerald-700 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-500/10">
                                  <UserCheck size={13} /> Accepter & Assigner
                                </button>
                                <button onClick={() => { setRefuseTarget(d); setActionMenu(null) }}
                                  className="w-full flex items-center gap-2.5 px-4 py-2 text-xs font-bold text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10">
        ...
        <div className="flex-shrink-0 flex items-center justify-between px-6 py-3 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-950">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-slate-500 dark:text-slate-400">Page {page} of {totalPages || 1}</span>
            <div className="flex items-center gap-1">
              {[
                { icon: <ChevronLeft size={13} className="-rotate-90" />, onClick: () => setPage(1),  disabled: page === 1, label: '«' },
                { icon: <ChevronLeft size={13} />,                        onClick: () => setPage(p => Math.max(1, p-1)), disabled: page === 1, label: '‹' },
                { icon: <ChevronRight size={13} />,                       onClick: () => setPage(p => Math.min(totalPages, p+1)), disabled: page >= totalPages, label: '›' },
                { icon: <ChevronRight size={13} className="-rotate-180" />, onClick: () => setPage(totalPages), disabled: page >= totalPages, label: '»' },
              ].map((btn, i) => (
                <button key={i} onClick={btn.onClick} disabled={btn.disabled}
                  className="w-7 h-7 rounded-lg border border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-500 dark:text-slate-400 disabled:opacity-30 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all text-xs font-bold">
                  {btn.label}
                </button>
              ))}
            </div>
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-2xl h-full shadow-2xl flex flex-col border-l border-slate-100 dark:border-slate-800"
            ...
            {detailLoading ? (
              <div className="flex-1 flex flex-col items-center justify-center gap-3">
                ...
                <p className="text-sm font-bold text-slate-400 dark:text-slate-500">Chargement des détails...</p>
              </div>
            ) : detailDecl ? (
              <>
                {/* Drawer Header */}
                <div className="flex-shrink-0 px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/50">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center text-lg shadow-sm">
                      {getCatEmoji(detailDecl.category)}
                    </div>
                    <div>
                      <h2 className="text-base font-black text-slate-900 dark:text-slate-100 leading-snug truncate max-w-[320px]">
                        {detailDecl.title}
                      </h2>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="font-mono text-[10px] font-black text-blue-600 dark:text-blue-400">
                          {detailDecl.ref_citoyen || '—'}
                        </span>
                        <span className="text-[10px] text-slate-400">•</span>
                        <span className="text-[10px] text-slate-500 dark:text-slate-400 font-bold">
                          {fmtDate(detailDecl.created_at)}
                        </span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => navigate('/chef/declarations')}
                      className="w-8 h-8 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 flex items-center justify-center text-slate-500 dark:text-slate-400 transition-all">
                      <X size={15} />
                    </button>
                  </div>
                </div>
                ...
                {/* Drawer Tabs */}
                <div className="flex-shrink-0 flex border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 px-6 gap-6">
                  {[
                    { id: 'info', label: 'Détails & Photos' },
                    { id: 'assign', label: 'Assignation' },
                    { id: 'history', label: 'Historique & Suivi' },
                    { id: 'comments', label: `Commentaires Internes (${comments.length})` },
                  ].map(tab => (
                    <button key={tab.id} onClick={() => setActiveTab(tab.id as any)}
                      className={`py-3.5 text-xs font-bold transition-all border-b-2 relative -mb-[2px] ${
                        activeTab === tab.id
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50/30 dark:bg-slate-900/10">
                  ...
                  {activeTab === 'info' && (
                    <div className="space-y-6">
                      ...
                      <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-100 dark:border-slate-800 shadow-sm space-y-3">
                        <h3 className="text-xs font-black text-slate-400 dark:text-slate-500 tracking-wider uppercase">Description</h3>
                        <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed font-medium">
                          {detailDecl.description || "Aucune description fournie."}
                        </p>
                        {detailDecl.address && (
                          <div className="flex items-start gap-2 pt-2 border-t border-slate-50 dark:border-slate-800 text-xs text-slate-500 dark:text-slate-400">
                            <span className="font-bold">📍 Adresse:</span>
                            <span>{detailDecl.address}</span>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-100 dark:border-slate-800 shadow-sm space-y-4">
                        <h3 className="text-xs font-black text-slate-400 dark:text-slate-500 tracking-wider uppercase">Signalement Citoyen</h3>
                        {detailDecl.citizen ? (
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              <p className="text-[10px] font-bold text-slate-400">Nom complet</p>
                              <p className="text-sm font-bold text-slate-800 dark:text-slate-200">
                                {detailDecl.citizen.first_name} {detailDecl.citizen.last_name}
                              </p>
                            </div>
                            <div>
                              <p className="text-[10px] font-bold text-slate-400">E-mail</p>
                              <p className="text-sm font-semibold text-slate-700 dark:text-slate-300 truncate">
                                {detailDecl.citizen.email}
                              </p>
                            </div>
                            <div>
                              <p className="text-[10px] font-bold text-slate-400">Téléphone</p>
                              <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">
                                {detailDecl.citizen.phone || '—'}
                              </p>
                            </div>
                            <div>
                              <p className="text-[10px] font-bold text-slate-400">Votes / Soutiens</p>
                              <p className="text-sm font-black text-blue-600 dark:text-blue-400">
                                {detailDecl.votes_count || 0} citoyen(s)
                              </p>
                            </div>
                          </div>
                        ) : (
                          <p className="text-xs text-slate-400 italic">Informations citoyen indisponibles.</p>
                        )}
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 border border-slate-100 dark:border-slate-800 shadow-sm space-y-2">
                          <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Photo Avant (Signalement)</h4>
                          <div className="aspect-[4/3] rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-800 relative flex items-center justify-center border border-slate-100 dark:border-slate-800">
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 border border-slate-100 dark:border-slate-800 shadow-sm space-y-2">
                          <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Photo Après (Résolution)</h4>
                          <div className="aspect-[4/3] rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-800 relative flex items-center justify-center border border-slate-100 dark:border-slate-800">
                            {detailDecl.photo_apres ? (
                              <img src={detailDecl.photo_apres} alt="Après" className="w-full h-full object-cover" />
                            ) : (
                              <div className="text-center p-3 space-y-1">
                                <Clock size={16} className="text-slate-300 dark:text-slate-600 mx-auto" />
                                <span className="block text-[10px] font-semibold text-slate-400">Non encore résolue</span>
                              </div>
                            )}
                          </div>
                        </div>
                  ...
                  {activeTab === 'assign' && (
                    <div className="space-y-6">
                      ...
                      <div className="bg-gradient-to-br from-slate-900 to-slate-950 dark:from-slate-900 dark:to-black rounded-2xl p-5 text-white space-y-4 shadow-xl border border-slate-800">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-300 mt-0.5">
                              En tant que chef de service, vous pouvez assigner cette tâche à un ou plusieurs agents de votre département.
                            </p>
                          </div>
                        </div>
                        {detailDecl.assigned_at && (
                          <div className="pt-3 border-t border-slate-800 text-[11px] text-slate-400 flex justify-between">
                            <span>Assignée le: <span className="text-white font-bold">{new Date(detailDecl.assigned_at).toLocaleString('fr-FR')}</span></span>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-100 dark:border-slate-800 shadow-sm space-y-4">
                        <div className="flex items-center justify-between">
                          <h3 className="text-xs font-black text-slate-400 dark:text-slate-500 tracking-wider uppercase">Agents de votre service</h3>
                          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400">
                            {selectedAgentIds.length} sélectionné(s)
                          </span>
                        </div>
                            ...
                            return (
                              <label key={agent.id}
                                className={`flex items-center gap-3.5 p-3.5 rounded-2xl border cursor-pointer transition-all ${
                                  isSelected
                                    ? 'border-blue-400 bg-blue-50/40 dark:bg-blue-500/5 dark:border-blue-500'
                                    : 'border-slate-100 dark:border-slate-800 hover:border-slate-200 dark:hover:border-slate-700 bg-white dark:bg-slate-900'
                                }`}>
                                ...
                                <div className="w-8.5 h-8.5 rounded-full flex items-center justify-center font-black text-xs text-white flex-shrink-0"
                                  style={{ background: agent.workload >= maxTasks ? '#ef4444' : '#3b82f6' }}>
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex items-center gap-2 mt-0.5">
                                    <div className="w-20 h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                                      <div className="h-full rounded-full" style={{
                                        width: `${Math.min(100, (agent.workload / maxTasks) * 100)}%`,
                                        background: agent.workload >= maxTasks ? '#ef4444' : agent.workload >= Math.ceil(maxTasks / 2) ? '#f59e0b' : '#22c55e'
                                      }} />
                                    </div>
                                    <span className="text-[10px] text-slate-400 font-bold">{agent.workload}/{maxTasks} tâches actives</span>
                                  </div>
                                </div>
                                {agent.workload >= maxTasks && (
                                  <span className="text-[9px] font-black text-red-500 border border-red-200 dark:border-red-500/30 rounded-full px-2 py-0.5 bg-red-50 dark:bg-red-500/10">
                                    Surcharge
                                  </span>
                                )}
                              </label>
                  ...
                  {/* Tab History */}
                  {activeTab === 'history' && (
                    <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-100 dark:border-slate-800 shadow-sm space-y-6">
                      <h3 className="text-xs font-black text-slate-400 dark:text-slate-500 tracking-wider uppercase">Suivi de Progression</h3>
                      
                      {history.length === 0 ? (
                        <p className="text-xs text-slate-400 italic">Aucun historique d'état disponible.</p>
                      ) : (
                          ...
                          {history.map((h, idx) => {
                            ...
                            return (
                              <div key={h.id || idx} className="relative">
                                {/* Dot indicator */}
                                <div className="absolute -left-[31px] top-1.5 w-4 h-4 rounded-full border-2 border-white dark:border-slate-950 flex items-center justify-center"
                                ...
                                <div className="space-y-1">
                                  ...
                                  {h.comment && (
                                    <p className="text-xs text-slate-600 dark:text-slate-400 font-semibold bg-slate-50 dark:bg-slate-900/50 p-2.5 rounded-xl border border-slate-100 dark:border-slate-800/60 leading-relaxed mt-1">
                                      {h.comment}
                                    </p>
                                  )}
                                  {h.user && (
                                    <span className="text-[9px] text-slate-400 dark:text-slate-500 block">
                                      Par: {h.user.first_name} {h.user.last_name} ({h.user.role})
                                    </span>
                                  )}
                                </div>
                              </div>
                  ...
                  {activeTab === 'comments' && (
                    <div className="space-y-4 flex flex-col h-full min-h-0">
                      ...
                      {/* Chat Messages */}
                      <div className="flex-1 bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-100 dark:border-slate-800 shadow-sm overflow-y-auto space-y-4 max-h-[300px]">
                        {comments.length === 0 ? (
                          <div className="py-12 text-center text-slate-400 dark:text-slate-500 italic space-y-2">
                            ...
                            <p className="text-[10px]">Commencez la discussion en ajoutant un commentaire ci-dessous.</p>
                          </div>
                        ) : (
                          comments.map((c, idx) => {
                      ...
                      {/* Chat Input form */}
                      <form onSubmit={handleAddComment} className="flex-shrink-0 flex items-center gap-2">
              ...
              <div className="flex-1 flex flex-col items-center justify-center gap-3">
                ...
                <p className="text-sm font-bold text-slate-400 dark:text-slate-500">Déclaration introuvable.</p>
              </div>
            )}
